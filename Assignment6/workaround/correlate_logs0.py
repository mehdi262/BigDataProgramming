...
def main(inputs):
    ...
    sum_df = cal_df.select(  cal_df['sum(one)'].alias('n') \
                            ,cal_df['sum(x)'].alias('sum_x') \
                            ,cal_df['sum(y)'].alias('sum_y') \
                            ,cal_df['sum(x2)'].alias('sum_x2') \
                            ,cal_df['sum(y2)'].alias('sum_y2') \
                            ,cal_df['sum(xy)'].alias('sum_xy'))
    ...
    result = final_nasa_df.select("*").rdd.flatMap(list).collect()
